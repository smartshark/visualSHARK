...
export default {
  ...
  methods: {
    initEditor: function() {
      ...
      const labelCssClass = ['bugfix', 'whitespace','documentation', 'refactoring', 'test', 'unrelated'];
      for(let line of this.file.lines) {
        if(line['new'] == '-') {
          this.originalLinesNeeded[line['old']] = line['number']
          if(line.label > 0) {
            this.setLabelLeft(line['old'], labelCssClass[line.label])
          }
        }
        if(line['old'] == '-') {
          this.modifiedLinesNeeded[line['new']] = line['number']
          if(line.label > 0) {
            this.setLabelRight(line['new'], labelCssClass[line.label])
          }
        }
      }
    },
    ...
    setLabelRight: function(line, label) {
      if(line in this.decorationsObjectsRigh) {
        this.decorationsObjectsRigh[line].options.linesDecorationsClassName = label
      }else {
        this.decorationsObjectsRigh[line] = {
          range: new monaco.Range(line, 1, line, 1),
          options: {
              isWholeLine: true,
              linesDecorationsClassName: label
