...
class Command(BaseCommand):
    ...
    def create_graphs(self, vcs_id):
        ...
        nodes = {}
        for c in Commit.objects.timeout(False).filter(vcs_system_ids=vcs_id).only('id', 'revision_hash', 'parents'):
        ...
        for c in Commit.objects.timeout(False).filter(vcs_system_ids=vcs_id).only('id', 'revision_hash', 'parents'):
            for p in c.parents:
                try:
                    p1 = Commit.objects.get(vcs_system_ids=vcs_id, revision_hash=p)
